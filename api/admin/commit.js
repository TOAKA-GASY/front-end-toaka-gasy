...
import { applyTextStyleEdits, parseTextStyles, serializeTextStyles, styleFilePath } from '../_lib/textStylesFile.js'
...
export default async function handler(req, res) {
  ...
  const { section, changes, styleChanges, code } = req.body || {}
  ...
  const hasStyleChanges = styleChanges && typeof styleChanges === 'object' && Object.keys(styleChanges).length > 0

  if (!section || typeof section !== 'string' || (!changes && !hasStyleChanges) || (changes && typeof changes !== 'object')) {
    res.status(400).json({ error: 'Requête invalide' })
    return
  }

  const localesToPatch = changes
    ? LOCALES.filter((locale) => changes[locale] && typeof changes[locale] === 'object' && Object.keys(changes[locale]).length > 0)
    : []

  if (localesToPatch.length === 0 && !hasStyleChanges) {
  ...
  try {
    ...
    if (hasStyleChanges) {
      const currentStyles = parseTextStyles(await getFileText(styleFilePath()))
      const updatedStyles = applyTextStyleEdits(currentStyles, styleChanges)
      files.push({ path: styleFilePath(), content: serializeTextStyles(updatedStyles) })
    }
